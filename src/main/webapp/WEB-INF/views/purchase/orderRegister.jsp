...
$(document).ready(function() {
	...
	$( "#tabs" ).tabs({
		//event: "mouseover" ,
	    collapsible: true      //탭을 펼치거나 접을 수 있게 하는 옵션이 true
...
function initGrid() {
	...
	$('#orderListGrid').jqGrid({ 
		datatype : 'json',
		datastr : gridRowJson,
		colNames : [ "선택","소요량취합번호","소요량전개번호","품목코드","품목명",
	...
	$('#orderDialogGrid').jqGrid({ 
		mtype : 'POST', 
		datatype : 'local',
		colNames : [ "아이템코드", "아이템이름", "단위", "총발주필요량", "사용가능재고량", 
			"실제발주필요량", "단가", "합계금액"], 
		colModel : [ 
...
function initEvent() {
	...
	$('#itemCodeSearchBox').on("click", function() {
		...
		showCodeDialog(this, "itemCodeSearchGrid", null , null , "IT-MA" , "품목코드 검색");  
	});
	...
	$('#orderButton').on("click", function() {
		...
		var mrpList = $('#orderDialogGrid').getRowData();
...
function showOrderDialog() {
	...
	$(checkedMrpRowIdList).each( function() {  
		...
		var rowId = $(this).val(); 
		var mrpNo = $('#orderListGrid').getRowData(rowId).mrpNo ; 
	...
	$.ajax({
		...
		data : {
			...
			mrpNoList : JSON.stringify(mrpNoList)    //소요량전개번호 리스트를 들고간다.
		},
		dataType : 'json', 
		cache : false, 
		success : function(dataSet) {
