...
var resultList = [];  // 최종적으로 컨트롤러로 보내는 JS 객체 배열. 즉, 일괄저장하기 전 보낼 값 전부.
...
function initGrid() {
		...
		colModel : [
			...
			{ name: "itemCode", width: "160", resizable: true, align: "left" , 
				formatter : function (cellvalue, options, rowObj) {
					...
					for( var i = 0 ; i < rowObj.bomLevel ; i++) {   //BOM 레벨만큼 스페이스바를 넣는다. 즉, 뷰단에서 계층형 보이기를 작업.
						str = space + str;
	...
	$('#bomInfoGrid').jqGrid({ 
		...
		colModel : [ 
			{ name: "bomDeleteCheck", width: "50", resizable: true, align: "center" ,
				formatter : function (cellvalue, options, rowObj) {
					...
					if( rowObj.no != 1 ) {   //가장 상위는 완제품이기 때문에 1 일 때는 else로.
						chk = "<input type='checkbox' name='bomDeleteCheck'  value=" +
							JSON.stringify(options.rowId) +" />";   //콘솔로그 찍어봤을 때 해당 input태그가 나타나는데, delete할 때 사용됨.
						return chk
					} else {
						return " ";   //완제품에 checkbox가 달리지 않게 했다.
					}
		...
		beforeSelectRow : function( rowid, event ) {
        	...
        	if($(event.target).is(":checkbox")) {   //이벤트가 일어난 곳이 체크박스 일 때,
            	
                if($(event.target).is(":checked")) {   //그 체크박스가 체크 되었을 때,
        			$(this).setCell(rowid,"status", "DELETE");    //status를 DELETE로 바뀌어라.
        			$(this).setCell(rowid,"beforeStatus", currentStatus);   //beforeStatus를 방금 바꾸기 전 status로 바꾸어라.
                } else {
		...
		beforeEditCell(rowid, cellname, value, iRow, iCol){
        	...
        	if(value == null || value == "" ) {
        		previousCellValue = " ";	//value가 없다면, previousCellValue(전에 cell값)을 spacebar로 세팅.
        	} else {
        		previousCellValue = value;  //있다면 value로 넣어라.
        	}
		...
		afterSaveCell(rowid, cellname, value, iRow, iCol){
        	...
        	var no = $(this).getCell(rowid,"no");   //no란, 동일 레벨 내 정렬 순서
        	
        	if( no == 1 && iCol == 2 && value == 1 ) {   //동일레벨의 1순위는 건들면 안되기 때문에
        		...
        		if( previousCellValue != value ) {     //이전값과 다른 값이 value에 있다면, 
    				$(this).setCell(rowid,"status", "UPDATE");    //status를 UPDATE로 바꿔라.
        		}
...
function initEvent() {
	...
	$('#searchBomDeployButton').on("click", function() {
		...
		if( itemClassificationCondition == 'IT-MA' && deployCondition == 'forward' ) {   // 원재료 선택 후 정전개시
			alertError("^^" , "원재료를 굳이 정전개할 필요는 없습니다.");
			return;
		}else if( itemClassificationCondition == 'IT-CI' && deployCondition == 'reverse' ) {  // 완제품 선택 후 역전개시
			alertError("^^" , "완제품을 굳이 역전개할 필요는 없습니다.");   //추가시킴
			return;
		}
	...
	$('#itemCodeSearchBox').on("click", function() {
		...
		showCodeDialog(this , "bomGrid1",  null , null , itemClassificationCondition, "품목 검색!!");
	...
	$('#parentItemCodeSearchBox').on("click", function() {
		...
		showCodeDialog(this , "bomGrid2",  null , null , itemClassificationCondition2, "품목 검색!!");
	...
	$('#bomInsertButton').on("click", function() {  //새로운 항목 추가 버튼 눌렀을 때,
		
		var ids = $('#bomInfoGrid').getRowData();   //BOM조회 한 이전 목록요소 들이 ids에 담김.

		var noArr = []; 
		
		$(ids).each(function(index, obj) {
			noArr.push(obj.no);  //목록요소들의 동일 레벨 내 정렬 순서를 array에 담는다.
		});
		
		var newRowNum = $('#bomInfoGrid').jqGrid('getDataIDs').length+1;  // 새로운 행 넘버
		
		$('#bomInfoGrid').addRowData(  //로우데이터를 추가한다
				newRowNum,   //새로운 행 넘버를
				{ "no" : Math.max.apply(null, noArr) + 1 , // ( no 중 최댓값 ) + 1 을 새로운 no 로 지정
										//null을 넣은 이유는 array에 값이 하나도 없을 때가 있을 수 있기 때문.
	...
	$('#batchSaveButton').on("click", function() {   //일괄저장 버튼을 눌렀을 때.
		
		var rowIdList =  $('#bomInfoGrid').jqGrid('getDataIDs'); //모든 ids 리스트를 받아옴.
		...
		$(rowIdList).each(function(index, rowId) {   // 전체 행에 대해 반복문 시작
			...
			var status = rowObject.status;  //행의 status
...
function checkRowChanged(previousRowValue, grid, rowid) {   //체크박스를 체크상태로 둘 때 status 를 변화시키는 로직.
