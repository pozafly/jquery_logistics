...
function initGrid() {
	...
	$('#workOrderListInfoGrid').jqGrid({ 
		...
		colNames : [ "선택","작업지시일련번호","소요량전개번호","주생산계획번호","소요량취합번호",
			"품목분류","품목코드","품목명","단위","작업지시한수량","작업완료된수량"], 
		colModel : [
...
function workOrderCompletion(){
	...
	$(checkedRow).each( function() { 
		...
		var rowId = $(this).val(); 
		workOrderNo = $('#workOrderListInfoGrid').getRowData(rowId).workOrderNo;
		actualCompletionAmount = $('#workOrderListInfoGrid').getRowData(rowId).actualCompletionAmount;
		
		if ( actualCompletionAmount == ""){ 
	...
	$.ajax({
		...
		success : function(dataSet) {
			...
			alertError("성공",workOrderNo + " 작업완료");
...
function showWorkOrderInfoListGrid(){
...
<div id = "workOrderDialog">
	...
	<fieldset style="display: inline;">
			<legend>작업지시 등록 일자</legend>
    			...
    			<input type="radio" name="workOrderDateCondition" value="workOrderDateAsToday" id="workOrderDateAsTodayRadio">
