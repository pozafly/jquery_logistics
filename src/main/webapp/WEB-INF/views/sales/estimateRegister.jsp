...
function initGrid() {
	...
	$('#estimateGrid').jqGrid({ 
		mtype : 'POST', 
		datatype : 'local',
		colNames : [ "거래처명","거래처코드", "견적일자", "유효일자","견적담당자","견적담당자코드", "견적요청자", "비고", "status" ], 
		colModel : [ 
		...
		onCellSelect : function(rowid, iCol, cellcontent, e) {
            ...
            alert(this, "estimateGrid", rowid , iCol , "CL-01","거래처 코드 검색");
	...
	$('#estimateDetailGrid').jqGrid({ 
		...
		colModel : [
			...
			{ name: "unitOfEstimate", width: "80", resizable: true, align: "center"} ,
...
function initEvent() {
	...
	$('#estimateDetailInsertButton').on("click",function() { //견적상세추가 이벤트
		...
		if( statusForInsertWorking == false ) {
			...
			var newRowNum = $('#estimateDetailGrid').jqGrid('getDataIDs').length+1;//견적상세누를떄마다 추가되는 열
			alert(newRowNum);
	...
	$('#batchSaveButton').on("click",function() {
		...
		var checkedDeleteList = $('#estimateDetailGrid').jqGrid('getGridParam','selarrrow'); //견적상세 그리드를 배열로 가져옴
