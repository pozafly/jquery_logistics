...
function initGrid() {
	...
	$('#contractCandidateGrid').jqGrid({ 
		mtype : 'POST', 
		datatype : 'local',
		colNames : [ "", "견적일련번호", "수주유형코드", "수주유형분류", "거래처코드", "거래처명", "견적일자", 
			"수주요청자", "유효일자", "견적담당자코드", "견적담당자", "비고"], 
		colModel : [ 
			...
			{ name: "contractType", width: "90", resizable: true, align: "center", hidden: true} ,
			{ name: "detailCodeName", width: "90", resizable: true, align: "center"},
			{ name: "customerCode", width: "80", resizable: true, align: "center"} ,
			{ name: "customerName", width: "100", resizable: true, align: "center", hidden: true} ,
			...
			{ name: "empCodeName", width: "100", resizable: true, align: "center" } ,
		...
		onSelectRow: function(rowid) { 
        	...
        	showEstimateDetailGrid();   //견적상세 그리드를 실행하는 메서드.
		...
		onCellSelect : function(rowid, iCol, previousCellValue, e) { 
        	...
        	if(iCol == 4) {        //수주유형분류 cell을 클릭했을 때.
				
            	showCodeDialog(this ,rowid , iCol , "CT", "수주 유형 분류")

			} if(iCol == 8 || iCol == 10) {   //수주요청자, 비고를 클릭했을 때.
	...
	$('#estimateDetailGrid').jqGrid({ 
		mtype : 'POST', 
		datatype : 'local',
		colNames : [ "견적상세일련번호", "품목코드", "품목명", "단위", "납기일", "견적수량", "견적단가", "합계액", "비고"], 
		colModel : [
...
function initEvent() {
	...
	$('#contractCandidateSearchButton').on("click", function() { //수주가능견적조회 버튼 이벤트
		...
		var searchCondition = $(':input:radio[name=searchDateCondition]:checked').val() //라디오가 체크 확인으로 true,false를 변수에 담음
	...
	$('#contractBatchInsertButton').on("click", function() {
		...
		console.log(checkedEstimateValue);
		// 데이터 유효성 검사
...
function showEstimateDetailGrid() {
	...
	$( gridRowJson ).each( function( index, obj )   {  // gridRowJson 의 전체 데이터에 대해 반복문 시작
		...
		if( obj.estimateNo == $('#contractCandidateGrid').jqGrid('getRowData', checkedRowId).estimateNo ) {
			...
			$('#estimateDetailGrid').jqGrid('setGridParam',{ 
				datatype : 'local', 
				data : obj.estimateDetailTOList 
			}).trigger('reloadGrid');
		} 
...
function showCodeDialog(grid, rowid, iCol, divisionCodeNo, title){
	...
	$("#codeGrid").jqGrid({
			...
			onSelectRow: function(id) {
				...
				if(codeUseCheck != 'N' && codeUseCheck != 'n') {     //codeUseCheck : 코드사용여부. 근데 구현안되어있음. 이로직 쓸모없음.
					
					$(grid).setCell(rowid, 'detailCodeName', detailName);
					$(grid).setCell(rowid, 'contractType', detailCode);
